...
void loop()
{
  if (radio.available())
  {
    ...
    {
     // Serial.print("$");
      Serial.print(data_1.temp_str); // в градусах
      Serial.println(" ");
  ...
  //    Serial.print(";");
    ...
    else if(!digitalRead(test_2)*!digitalRead(test_1))
    {
      Serial.print("GPS lon :  ");            //
      Serial.print(data_1.gps_lon);                     // пишем в порт температуру
      Serial.print("    GPS lat :  ");                //
      Serial.println(data_1.gps_lat);                  // пишем в порт давление с барометра
    ...
    else
    {
      ...
      Serial.print(" ");                      //
      Serial.println(data_1.x_str / 3.36);
      Serial.print(" ");  // пишем в порт ускорение по оси X
      Serial.print("Y (adxl345):");                      //
      Serial.println(data_1.y_str / 3.36);
      Serial.print(" ");  // пишем в порт ускорение по оси Y
      Serial.print("Z (adxl345):");                      //
      Serial.println(data_1.z_str / 3.36);                      // пишем в порт ускорение по оси Z
      Serial.print("GPS lon lat:");
      Serial.print(data_1.gps_lon, 5);
      Serial.print("     ");
      Serial.println(data_1.gps_lat, 5);
      Serial.print("O2 % : ");
