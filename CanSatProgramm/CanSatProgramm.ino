...
void loop()
{
  ...
  if ((data_1.timer % 3 == 0))        //  раз в 3 итерации выполняется снятие данных с датчика
  {                                 //  
  ...
  data_1.z_str = z;                                   /////////////////////////////////////////////////////////////
  get_MH_Z14A_data(data_2.CO2_ppm, data_2.MH_Z14A_temp);// получение данных с датчика CO2
  gps.read(); 
  if(gps.errPos){}
  else 
  {
...
boolean get_MH_Z14A_data(int16_t &ppm, int8_t &temp)
{
  ...
  if(Serial.readBytes(get_value, 9) == 9)
  {
